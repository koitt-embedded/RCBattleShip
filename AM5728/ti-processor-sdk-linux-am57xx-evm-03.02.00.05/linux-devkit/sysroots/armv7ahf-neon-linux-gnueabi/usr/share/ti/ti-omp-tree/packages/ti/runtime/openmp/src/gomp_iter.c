...
/* No need for locking because workshare is not modified */
...
gomp_iter_static_next (long *pstart, long *pend)
{
  ...
  if (thr->ts->static_trip == (unsigned long)-1)
    return -1;
  ...
  if (nthreads == 1)
    {
      *pstart = ws->next;
      *pend = ws->end;
      thr->ts->static_trip = (unsigned long)-1;
...
gomp_iter_guided_next_locked (long *pstart, long *pend)
{
  ...
  *pstart = start;
  *pend = end;
  tomp_logDebug3("TRACE: Guided %d, [%d,%d)\n", thr->ts->team_id, start, end);
  return true;
}
