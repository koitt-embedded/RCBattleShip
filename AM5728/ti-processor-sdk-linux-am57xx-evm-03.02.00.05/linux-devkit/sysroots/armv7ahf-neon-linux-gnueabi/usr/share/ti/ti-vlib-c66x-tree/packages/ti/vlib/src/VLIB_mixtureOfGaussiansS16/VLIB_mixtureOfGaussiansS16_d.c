...
#include "VLIB_mixtureOfGaussiansS16_cn.h"
#include "VLIB_mixtureOfGaussiansS16_idat.h"

/* VLIB_mixtureOfGaussiansS16_d:  Test Driver Routine */
void VLIB_mixtureOfGaussiansS16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    mixtureOfGaussiansS16_testParams_t   *prm;

    mixtureOfGaussiansS16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_mixtureOfGaussiansS16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        int16_t    *mu          =  (int16_t *) VLIB_malloc(prm[tpi].im_size * 3 * sizeof(int16_t));
        int16_t    *var         =  (int16_t *) VLIB_malloc(prm[tpi].im_size * 3 * sizeof(int16_t));
        ...
        if( Im && wt && mu && var && compNum && updateMask8 && Im_mask && Im_mask_cn) {
            ...
            memset(mu, 0, prm[tpi].im_size * 3 * sizeof(int16_t));
            memset(var, 0, prm[tpi].im_size * 3 * sizeof(int16_t));
            counter = 0;
            ...
            for( i=0; i < prm[tpi].num_images; i++ ) {
                ...
                VLIB_mixtureOfGaussiansS16(&Im[i*prm[tpi].im_size],
                ...
                if( LevelOfFeedback > 0 ) {
                    printf("%4d  %d  %c %8.4f %8.4f %8.6f   %c %8.4f %8.4f %8.6f   %c %8.4f %8.4f %8.6f\n", Im[i*prm[tpi].im_size], (Im_mask[0] & 0x80000000) >> 31,
                           mode[0], VLIB_getFractional16(mu[0], 7, 1), VLIB_getFractional16(var[0], 3, 1), VLIB_getFractional16(wt[0], 15, 1),
                           mode[1], VLIB_getFractional16(mu[0 + (prm[tpi].im_size)], 7, 1), VLIB_getFractional16(var[0 + (prm[tpi].im_size)], 3, 1), VLIB_getFractional16(wt[0 + (prm[tpi].im_size)], 15, 1),
                           mode[2], VLIB_getFractional16(mu[0 + 2 * (prm[tpi].im_size)], 7, 1), VLIB_getFractional16(var[0 + 2 * (prm[tpi].im_size)], 3, 1), VLIB_getFractional16(wt[0 + 2 * (prm[tpi].im_size)], 15, 1));
                }
            ...
            for( i=0; i < prm[tpi].num_images; i++ ) {
                ...
                VLIB_mixtureOfGaussiansS16_cn(&Im[i*prm[tpi].im_size],
                ...
                if( LevelOfFeedback > 0 ) {
                    printf("%4d  %d  %c %8.4f %8.4f %8.6f   %c %8.4f %8.4f %8.6f   %c %8.4f %8.4f %8.6f\n", Im[i*prm[tpi].im_size], (Im_mask_cn[0] & 0x80000000) >> 31,
                           mode[0], VLIB_getFractional16(mu[0], 7, 1), VLIB_getFractional16(var[0], 3, 1), VLIB_getFractional16(wt[0], 15, 1),
                           mode[1], VLIB_getFractional16(mu[0 + (prm[tpi].im_size)], 7, 1), VLIB_getFractional16(var[0 + (prm[tpi].im_size)], 3, 1), VLIB_getFractional16(wt[0 + (prm[tpi].im_size)], 15, 1),
                           mode[2], VLIB_getFractional16(mu[0 + 2 * (prm[tpi].im_size)], 7, 1), VLIB_getFractional16(var[0 + 2 * (prm[tpi].im_size)], 3, 1), VLIB_getFractional16(wt[0 + 2 * (prm[tpi].im_size)], 15, 1));
                }
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_mixtureOfGaussiansS16");
    } else {
        VLIB_mixtureOfGaussiansS16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_mixtureOfGaussiansS16_d.c                            */
/* ======================================================================== */
