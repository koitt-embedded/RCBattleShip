...
#include "../VLIB_kalmanFilter_4x6_Predict_S16_F32/VLIB_kalmanFilter_4x6_Predict_S16_F32_cn.h"

#include "VLIB_kalmanFilter_4x6_Correct_S16_F32.h"
#include "VLIB_kalmanFilter_4x6_Correct_S16_F32_cn.h"
#include "VLIB_kalmanFilter_4x6_Correct_S16_F32_idat.h"

/* VLIB_kalmanFilter_4x6_Correct_F32_d:  Test Driver Routine */
void VLIB_kalmanFilter_4x6_Correct_S16_F32_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    kalmanFilter_4x6_Correct_S16_F32_testParams_t   *prm;

    kalmanFilter_4x6_Correct_S16_F32_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_kalmanFilter_4x6_Correct_S16_F32");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        VLIB_kalmanFilter_4x6_S16_F32   *KF          = (VLIB_kalmanFilter_4x6_S16_F32 *) VLIB_memalign(8, sizeof(VLIB_kalmanFilter_4x6_S16_F32));
        VLIB_kalmanFilter_4x6_S16_F32   *KF_cn       = (VLIB_kalmanFilter_4x6_S16_F32 *) memalign(8, sizeof(VLIB_kalmanFilter_4x6_S16_F32));
        ...
        if( Z && Z_buffer && Residual && Residual_cn && KF && KF_cn ) {
            ...
            memset(KF, 0, sizeof(VLIB_kalmanFilter_4x6_S16_F32));
            memset(KF_cn, 0, sizeof(VLIB_kalmanFilter_4x6_S16_F32));
            ...
            for( i=0; i < prm[tpi].numMeasurements; i++ ) {
                ...
                /* kalman predict */
                VLIB_kalmanFilter_4x6_Predict_S16_F32_cn(KF);  // The natC version is used so that the code size doesn't get counted
                VLIB_kalmanFilter_4x6_Predict_S16_F32_cn(KF_cn);

                /* get measurement */
                Z[0] = Z_buffer[i * mD_4x6 + 0] + 0.5;
                Z[1] = Z_buffer[i * mD_4x6 + 1] + 0.5;
                Z[2] = Z_buffer[i * mD_4x6 + 2] + 0.5;
                Z[3] = Z_buffer[i * mD_4x6 + 3] + 0.5;

                /* Test optimized kernel */
                VLIB_profile_start(vlib_KERNEL_OPT);
                VLIB_kalmanFilter_4x6_Correct_S16_F32(KF, Z, Residual);
                VLIB_profile_stop();

                /* Test _cn kernel */
                VLIB_profile_start(vlib_KERNEL_CN);
                VLIB_kalmanFilter_4x6_Correct_S16_F32_cn(KF_cn, Z, Residual_cn);
                VLIB_profile_stop();
                ...
                for( j=0; j < 6; j++ ) {
                    ...
                    if( prm[tpi].staticOut != NULL ) {
                        if( fabs(KF_cn->state[j] - prm[tpi].staticOut[(i * 6) + j]) > 4 ) {
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_kalmanFilter_4x6_Correct_S16_F32");
    } else {
        VLIB_kalmanFilter_4x6_Correct_S16_F32_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_kalmanFilter_4x6_Correct__S16_F32_d.c                */
/* ======================================================================== */
