...
#include "VLIB_originalfast12_detectCorners_cn.h"
#include "VLIB_originalfast12_detectCorners_idat.h"

void VLIB_originalfast12_detectCorners_d(uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    originalfast12_detectCorners_testParams_t   *prm;

    originalfast12_detectCorners_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_originalfast12_detectCorners");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        CORBOutput   *orbOutput     = (CORBOutput *) malloc(sizeof(CORBOutput));
        ...
        if( dummyPtr && orbOutput->features && orbOutput_cn->features && orbInput->prefimage ) {
            ...
            VLIB_originalfast12_detectCorners(orbInput->prefimage, orbParams.width, orbParams.height, prm[tpi].imagePitch, orbParams.marginPixels,
                                              orbParams.fastDifferenceThreshold, &orbOutput->n_features, orbOutput->features);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_originalfast12_detectCorners_cn(orbInput->prefimage, orbParams.width, orbParams.height, prm[tpi].imagePitch, orbParams.marginPixels,
                                                 orbParams.fastDifferenceThreshold, &orbOutput_cn->n_features, orbOutput_cn->features);
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_originalfast12_detectCorners");
    } else {
        VLIB_originalfast12_detectCorners_d(1);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_originalfast12_detectCorners_d.c                     */
/* ======================================================================== */
