  ...
  if (gomp_work_share_start (false))
    {
      ...
      /* Initializes pointer to allocated workshare via gomp_ptrlock_set */
...
/* schedule(runtime) requires the use of an environment variable to
 * specify scheduling and chunk size. Since the TI runtime does not support 
 * env vars, use defaults - run_sched_var is dynamic and rn_sched_modifier is 1
 */
bool
			  ...
			  long chunk_size)
{
  ...
  gomp_loop_init (&team->dummy_ws, start, end, incr, sched, chunk_size);
