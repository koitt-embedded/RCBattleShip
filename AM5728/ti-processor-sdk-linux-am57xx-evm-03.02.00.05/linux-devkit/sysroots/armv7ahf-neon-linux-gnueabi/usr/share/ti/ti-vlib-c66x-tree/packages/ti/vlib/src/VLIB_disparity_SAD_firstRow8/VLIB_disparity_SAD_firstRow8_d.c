...
#include "../VLIB_disparity_SAD8/VLIB_disparity_SAD8_cn.h"

#include "VLIB_disparity_SAD_firstRow8.h"
#include "VLIB_disparity_SAD_firstRow8_cn.h"
#include "VLIB_disparity_SAD_firstRow8_idat.h"

/* VLIB_disparity_SAD_firstRow8_d:  Test Driver Routine */
void VLIB_disparity_SAD_firstRow8_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    disparity_SAD_firstRow8_testParams_t   *prm;

    disparity_SAD_firstRow8_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_disparity_SAD_firstRow8");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( pLeft && pRight && pCost && pMinCost && pScratch && pDisparity && pDisparity_cn && maxWidth > 0) {
            ...
            for(i=0; i<padWidth; i++) pMinCost[i] = 0x7FFF;
            ...
            for(d=prm[tpi].minDisp; d<=prm[tpi].maxDisp; d++)
            {
                ...
                VLIB_disparity_SAD_firstRow8((uint8_t *)&pLeft[idxIn], (uint8_t *)&pRight[idxIn-d],
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_disparity_SAD_firstRow8");
    } else {
        VLIB_disparity_SAD_firstRow8_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_disparity_SAD_firstRow8_d.c                          */
/* ======================================================================== */
