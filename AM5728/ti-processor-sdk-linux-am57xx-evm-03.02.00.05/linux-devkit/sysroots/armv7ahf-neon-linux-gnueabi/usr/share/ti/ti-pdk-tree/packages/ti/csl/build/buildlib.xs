 ...
 *
 * DESCRIPTION: 
 *  This file contains common routines that are used by the various CSL
 *  components.
 *
...
function createMake(makefile)
{
    ...
    if(makefile==undefined)
    {
      ...
      */
      ...
      makefile.writeLine("export BINDIR ?= ./bin");
      makefile.writeLine("export ARMV7LIBDIR ?= $LIBDIR/a15/release");
      makefile.writeLine("export ARMV7OBJDIR ?= $OBJDIR/static");
      makefile.writeLine("export ARMV7OBJDIR_SO ?= $OBJDIR/dynamic");
      ...
      makefile.writeLine("\n# INCLUDE Directory");
      makefile.writeLine("export INCDIR := "+getxdcpath+";$(ROOTDIR);$(ROOTDIR)/ti/csl;$(ROOTDIR)/ti/csl/src/ip/i2c/V2;$(ROOTDIR)/ti/csl/arch/m4");
      ...
      /* Write the rule to make test in main makefile */
      makefile.writeLine("\n\n# Make rule to create $(BINDIR)/am572x/armv7/wdt_test_am572x.xa15fg test");
      makefile.writeLine(".tests: $(BINDIR)/am572x/armv7/wdt_test_am572x.xa15fg");
      makefile.writeLine("$(BINDIR)/am572x/armv7/wdt_test_am572x.xa15fg: FORCE\n\t$(MAKE) -f ./test/wdt/am572x/armv7/wdt_test_am572x.xa15fg.mk $@");

      /* Rule to clean test in main makefile */
      makefile.writeLine("# Rule to clean $(BINDIR)/am572x/armv7/wdt_test_am572x.xa15fg test");
      makefile.writeLine("clean ::\n\t$(RM) $(BINDIR)/am572x/armv7/wdt_test_am572x.xa15fg");

      /* Write the rule to make test in main makefile */
      makefile.writeLine("\n\n# Make rule to create $(BINDIR)/am572x/armv7/rtc_test_am572x.xa15fg test");
      makefile.writeLine(".tests: $(BINDIR)/am572x/armv7/rtc_test_am572x.xa15fg");
      makefile.writeLine("$(BINDIR)/am572x/armv7/rtc_test_am572x.xa15fg: FORCE\n\t$(MAKE) -f ./test/rtc/am572x/armv7/rtc_test_am572x.xa15fg.mk $@");

      /* Rule to clean test in main makefile */
      makefile.writeLine("# Rule to clean $(BINDIR)/am572x/armv7/rtc_test_am572x.xa15fg test");
      makefile.writeLine("clean ::\n\t$(RM) $(BINDIR)/am572x/armv7/rtc_test_am572x.xa15fg");
   }
   else
   {
     try{
...
function createLibMake(makelibname, evmName, targetname, objectPath)
{
   var tooldir;
   var cmdprefix;
   var targetDir;
...
*/   
   ...
   switch(stringname)
   {
    ...
    if ((targetname.name == "A15F") || (targetname.name == "A9F") || (targetname.name == "A8F"))
    {
        ...
        libmakefile.writeLine("INCS = -I. -I$("+tooldir+")/include -I$(strip $(subst ;, -I,$(subst $(space),\\$(space),$(INCDIR))))");
        ...
        libmakefile.writeLine("OBJDIR =  ./obj/obj_" +targetname.suffix +"/"+evmName +targetDir+""); 
    }
    else
    {
...
function makeAddObjects(srcString, makefilename, srcfiles, flags,fileExt, targetName)
{
 ...
 if (targetName.name == "A15F")
 {
   ...
   {
    makefilename.writeLine(objectliststring +" = $(patsubst %."+fileExt+", $(OBJDIR)/%.$(AOBJEXT), $(" + sourcestring + "))"); 
    makefilename.writeLine("\n$("+objectliststring+"): $(OBJDIR)/%.$(AOBJEXT): %."+fileExt);   
   }
 }
 else
 {
 ...
 if(fileExt == "c")
 {
   if ((targetName.name == "A15F") || (targetName.name == "A9F") || (targetName.name == "A8F"))
   {
    ...
    makefilename.writeLine("\t$(CC) -x none $(CFLAGS_INTERNAL) $("+compileflagstring+") $(INTERNALDEFS) $(INCS) $< -o $@");
   /* 
    TBD
   */
   }
 ...
 else if(fileExt == "asm")
 {
   ...
   {
    makefilename.writeLine("\t$(RM) $@.dep");
    makefilename.writeLine("\t$(CC) -x assembler $(CFLAGS_INTERNAL) $("+compileflagstring+") $(INTERNALDEFS) $(INCS) $< -o $@");
   }
   else
   {
 ...
 if ((targetName.name == "A15F") || (targetName.name == "A9F") || (targetName.name == "A8F"))
 {
   ...
   {
    makefilename.writeLine(" -include $("+objectliststring+":%.$(AOBJEXT)=%.$(AOBJEXT).dep)");
   }
 }
 else
 {
...
function buildLibrary (libOptions, libName, evmName, target, libFiles) 
{
    ...
    var makefilelib= createLibMake(lib+".mk",evmName +"/",target,objectPath);  
