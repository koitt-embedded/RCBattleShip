...
#include "VLIB_dilate_bin_square_cn.h"
#include "VLIB_dilate_bin_square_idat.h"

#include "../VLIB_packMask32/VLIB_packMask32_cn.h"
#include "../VLIB_unpackMask32/VLIB_unpackMask32_cn.h"

/* VLIB_dilate_bin_square_d:  Test Driver Routine */
void VLIB_dilate_bin_square_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    dilate_bin_square_testParams_t   *prm;

    dilate_bin_square_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_dilate_bin_square");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( input_unpk && in_data && out_data && output_unpk && output_unpk_cn ) {
            ...
            VLIB_dilate_bin_square(in_data, out_data + prm[tpi].in_width / 8, cols, pitch); /* Center output vertically */
            VLIB_profile_stop();
            ...
            VLIB_dilate_bin_square_cn(in_data, out_data + prm[tpi].in_width / 8, cols, pitch); /* Center output vertically */
            VLIB_profile_stop();
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_dilate_bin_square");
    } else {
        VLIB_dilate_bin_square_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_dilate_bin_square_d.c                                */
/* ======================================================================== */
