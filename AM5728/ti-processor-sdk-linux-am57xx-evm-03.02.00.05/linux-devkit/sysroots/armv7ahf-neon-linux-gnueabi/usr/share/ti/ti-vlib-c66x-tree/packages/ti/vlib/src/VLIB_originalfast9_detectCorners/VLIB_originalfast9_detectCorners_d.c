...
#include "VLIB_originalfast9_detectCorners_cn.h"
#include "VLIB_originalfast9_detectCorners_idat.h"

void VLIB_originalfast9_detectCorners_d(uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    originalfast9_detectCorners_testParams_t   *prm;

    originalfast9_detectCorners_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(2, "VLIB_originalfast9_detectCorners");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( dummyPtr && orbOutput->features && orbOutput_cn->features && orbInput->prefimage ) {
            ...
            VLIB_originalfast9_detectCorners(orbInput->prefimage, orbParams.width, orbParams.height, prm[tpi].imagePitch, orbParams.marginPixels,
                                              orbParams.fastDifferenceThreshold, &orbOutput->n_features, orbOutput->features);
            VLIB_profile_stop();

            /* Test _cn kernel */
            VLIB_profile_start(vlib_KERNEL_CN);
            VLIB_originalfast9_detectCorners_cn(orbInput->prefimage, orbParams.width, orbParams.height, prm[tpi].imagePitch, orbParams.marginPixels,
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_originalfast9_detectCorners");
    } else {
        VLIB_originalfast9_detectCorners_d(1);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_originalfast9_detectCorners_d.c                      */
/* ======================================================================== */
