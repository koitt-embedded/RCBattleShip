...
#include "../VLIB_disparity_SAD16/VLIB_disparity_SAD16_cn.h"

#include "VLIB_disparity_SAD_firstRow16.h"
#include "VLIB_disparity_SAD_firstRow16_cn.h"
#include "VLIB_disparity_SAD_firstRow16_idat.h"

/* VLIB_disparity_SAD_firstRow16_d:  Test Driver Routine */
void VLIB_disparity_SAD_firstRow16_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    disparity_SAD_firstRow16_testParams_t   *prm;

    disparity_SAD_firstRow16_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_disparity_SAD_firstRow16");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( pLeft && pRight && pCost && pMinCost && pScratch && pDisparity && pDisparity_cn && maxWidth > 0) {
            ...
            for(i=0; i<padWidth; i++) pMinCost[i] = 0x7FFFFFFF;
            ...
            for(d=prm[tpi].minDisp; d<=prm[tpi].maxDisp; d++)
            {
                ...
                VLIB_disparity_SAD_firstRow16((uint16_t *)&pLeft[idxIn], (uint16_t *)&pRight[idxIn-d],
            ...
            VLIB_formula_add_test(padWidth*numDisp, numDisp, NULL, fail, desc, NULL);
    ...
    VLIB_profile_cycle_report(vlib_PROFILE_FORMULA_RANGE,
                              "N = padWidth*(disparitySearchRange); M = disparitySearchRange",
                              "padWidth*(disparitySearchRange)");
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_disparity_SAD_firstRow16");
    } else {
        VLIB_disparity_SAD_firstRow16_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_disparity_SAD_firstRow16_d.c                         */
/* ======================================================================== */
