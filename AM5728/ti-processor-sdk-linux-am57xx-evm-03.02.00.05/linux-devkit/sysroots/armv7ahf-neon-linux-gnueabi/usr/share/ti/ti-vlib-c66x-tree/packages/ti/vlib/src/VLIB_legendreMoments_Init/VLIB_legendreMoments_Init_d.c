...
#include "VLIB_legendreMoments_Init_cn.h"
#include "VLIB_legendreMoments_Init_idat.h"

#include "../VLIB_legendreMoments/VLIB_legendreMoments_cn.h"

/* VLIB_legendreMoments_Init_d:  Test Driver Routine */
void VLIB_legendreMoments_Init_d (uint8_t LevelOfFeedback)
{
    int32_t    tpi;  /* test parameter index */

    /* Test Parameters */
    legendreMomentsInit_testParams_t   *prm;

    legendreMomentsInit_getTestParams(&prm, &test_cases);

    /* Initialize profiling */
    VLIB_profile_init(3, "VLIB_legendreMoments_Init");
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        if( Im && LPoly && LMoments && LMoments_cn) {
            ...
            VLIB_legendreMoments_Init(LPoly, prm[tpi].Order, prm[tpi].ImH, &Factor1);
            VLIB_profile_stop();

            VLIB_legendreMoments_cn(Im, LMoments, LPoly, prm[tpi].Order, prm[tpi].ImH, Factor1);
            ...
            VLIB_formula_add_test((prm[tpi].Order-1)*prm[tpi].ImH, prm[tpi].ImH, NULL, fail, desc, NULL);
...
main() {
    if( VLIB_cache_init()) {
        VLIB_memError("VLIB_legendreMoments_Init");
    } else {
        VLIB_legendreMoments_Init_d(0);
    }
}
#endif /* __ONESHOTTEST */

/* ======================================================================== */
/*  End of file:  VLIB_legendreMoments_Init_d.c                             */
/* ======================================================================== */
