...
void VLIB_ORB_computeOrientation_d(uint8_t LevelOfFeedback)
{
    ...
    for( tpi=0; tpi < test_cases; tpi++ ) {
        ...
        int16_t   *outCosAngles      = (int16_t *) VLIB_memalign(4, nfeatures * sizeof(int16_t));
        int16_t   *outSineAngles     = (int16_t *) VLIB_memalign(4, nfeatures * sizeof(int16_t));

        int16_t   *outCosAngles_cn   = (int16_t *) malloc(nfeatures * sizeof(int16_t));
        int16_t   *outSineAngles_cn  = (int16_t *) malloc(nfeatures * sizeof(int16_t));

        uint8_t   *scratch           = (uint8_t *) VLIB_malloc(1991);

        if( featuresLoc && inImage && outCosAngles && outSineAngles && outCosAngles_cn && outSineAngles_cn && scratch) {
            ...
            for( i=0; i < cnt; i++ ) {
                if((outCosAngles[i] != outCosAngles_cn[i]) ||
                   (outSineAngles[i] != outSineAngles_cn[i])) {
            ...
            if((prm[tpi].testPattern == STATIC) &&
               ...
               (prm[tpi].staticOut2 != NULL)) {
                for( i=0; i < cnt; i++ ) {
                    if((outCosAngles_cn[i] != prm[tpi].staticOut1[i]) ||
                       (outSineAngles_cn[i] != prm[tpi].staticOut2[i])
                       ) {
        ...
        VLIB_free(scratch);
        VLIB_align_free(outCosAngles);
        VLIB_align_free(outSineAngles);
        free(outCosAngles_cn);
        free(outSineAngles_cn);
