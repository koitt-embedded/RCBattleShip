...
static void help()
{
/* 이 프로그램은 <ORB BRISK> 및 <AKAZE 특징량>을 compute하고 match시키는 방법을 보여줍니다. */
...
/* 알고리즘이나 특징량을 변경하기 위해 이미지 창이 활성화되어 있을 때 키를 누르십시오 */
        "Press a key when image window is active to change algorithm or descriptor";
}
/* 특징점(keypoint), 특징량(descriptor), 매칭(matcher) , 이 3가지는 서로 조합적으로 선택하여 사용할 수 있으며, 이에 관한 예제코드이다. */
