...
Notes: 
	...
	주어진 ρ, 쎄타의 임계 값 내에서 발견되는 라인에 숫자를 더하고 그 값을 사용하여 점수를 결정하십시오.
	스코어가 충분한 라인 만 유지됩니다.
        센터에서 차량 거리에 대한 계산. 이것은 선회에서 도로가 있는지를 결정해야합니다.
	우리는 길의 중심에 있고 싶지 않을 것입니다.
	...
	몇 가지 다른 매개 변수를 사용할 수 있습니다 : houghp, 회선 거리 및 간격에 대한 최소 투표. 피드 백 루프의 일부 유형은 이러한 매개 변수를 자체 조정할 때 유용 할 수 있습니다.
         우리는 여전히 도로, 즉 좌우 차선을 찾고 있습니다. 중간에 노란색 구분선이 보이도록 설정해야합니다.
         수평 및 수직선을 줄이기 위해 쎄타 각에 필터를 추가했습니다.
         나무 / 전력선과 같은 것들로부터 가짜 라인을 줄이기 위해 이미지 ROI를 추가했습니다.
\*------------------------------------------------------------------------------------------*/
