 ...
 * to_bank() macro below.
  ...
  * struct tz1090_gpio_bank - GPIO 은행 개인 데이터
  * @ 칩 : GPIO 뱅크 용 일반 GPIO 칩
  * @ 도메인 : GPIO 뱅크 용 IRQ 도메인 (NULL 일 수 있음)
  * @reg :이 GPIO 뱅크에 대한 오프셋 레지스터
  * @irq : GPIO 뱅크의 IRQ 번호
  * @label : 칩 -> 라벨의 저장에 사용되는 GPIO 은행 라벨 디버그
  *
  * 이것은 GPIO 은행의 주요 개인 데이터입니다. gpio_chip을 캡슐화하고 gpio_chip의 콜백은 아래 to_bank () 매크로를 사용하여 개인 데이터에 액세스 할 수 있습니다.
 */
 ...
 * temporary, and used during init.
  ...
  * struct tz1090_gpio - 전체 GPIO 장치 개인 데이터
  * @dev : 장치 (플랫폼 장치에서)
  * @reg : GPIO 레지스터의베이스
  *
  * 전체 GPIO 장치를 나타냅니다. 이 구조체는 실제로는 일시적이며 init 중에 사용됩니다.
 */
struct tz1090_gpio {
	struct device *dev;
	void __iomem *reg;
 ...
 * @index:	Index of bank in range 0-2
  ...
  * struct tz1090_gpio_bank_info - GPIO 은행 임시 등록 정보
  * @priv : 전체 GPIO 장치 개인 데이터
  * @node :이 GPIO 뱅크에 특정한 장치 트리 노드
  * @ 인덱스 : 0-2 범위의 뱅크 색인
 */
...
/* Get TZ1090 GPIO chip from irq data provided to generic IRQ callbacks 
   일반 IRQ callback에 제공된 irq 데이터에서 TZ1090 GPIO 칩 가져 오기		*/
...
/* set polarity to trigger on next edge, whether rising or falling
   상승 또는 하강 여부에 관계없이 다음 edge에서 trigger 하도록 극성 설정  */
				      ...
				      unsigned int offset)
{
	 ...
	 * input value so that the next edge triggers an interrupt.
    	   다음 에지가 인터럽트를 트리거하도록 GPIO의 인터럽트 극성을 현재 입력 값의 반대 방향으로 설정하십시오.
	 */
	__global_lock2(lstat);
...
static unsigned int gpio_startup_irq(struct irq_data *data)
{
	 ...
	 * trigger flags to request_irq().
	   이 경고는 irq를 활성화하기 전에 irq 유형이 설정되지 않았 음을 나타냅니다. 
	   이는 일반적으로 request_irq ()에 일부 트리거 플래그를 전달하여 수행됩니다.
	 */
...
static int tz1090_gpio_bank_probe(struct tz1090_gpio_bank_info *info)
{
	...
	/* Offset the main registers to the first register in this bank
	   이 뱅크의 첫번째 레지스터에 메인 레지스터를 오프셋한다.			 */
	...
	/* GPIO numbering from 0 	0부터 GPIO 번호 매기기 	*/
	bank->chip.base			= info->index * 30;
	bank->chip.ngpio		= 30;

	/* Add the GPIO bank 		 GPIO 뱅크 추가		*/
	gpiochip_add(&bank->chip);

	/* Get the GPIO bank IRQ if provided 
	   제공되는 경우 GPIO 뱅크 IRQ 가져 오기			*/
	bank->irq = irq_of_parse_and_map(np, 0);

	/* The interrupt is optional (it may be used by another core on chip) 
	   인터럽트는 선택 사항입니다 (칩의 다른 코어에서 사용할 수 있습니다) 	*/
	if (!bank->irq) {
	 ...
	 * handler.
	모든 인터럽트를 비활성화로 설정하여 더티 부팅시 위조 된 버그를 일으키지 않고 핸들러의 BUG_ON을 누르십시오.
	 */
	tz1090_gpio_write(bank, REG_GPIO_IRQ_EN, 0);

	/* Add a virtual IRQ for each GPIO 
	   각 GPIO에 대한 가상 IRQ 추가			*/
	...
	/* Set up a generic irq chip with 2 chip types (level and edge)
	   2 가지 칩 타입 (레벨과 에지)을 가진 일반적인 irq 칩을 설정하십시오.	 */
