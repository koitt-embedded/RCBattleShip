 ...
 * XLP GPIO는 각 레지스터가 32 핀을 제어하는 각 기능마다 다중 32 비트 레지스터를 가지고 있습니다.
 * 따라서 최대 64 개의 핀에는 2 개의 32 비트 레지스터가 필요하며 각 기능에는 3 개의 32 비트 레지스터가 필요합니다.
 * 여기서 각 피쳐의 첫 번째 레지스터 오프셋 만 정의합니다. 32보다 큰 핀에 대한 레지스터의 오프셋은 다음과 같이 계산할 수 있습니다 (예 : GPIO_INT_STAT 사용).
 *
 * 오프셋 = (gpio / XLP_GPIO_REGSZ) * 4;
 * reg_addr = addr + 오프셋;
 *
 * addr은 해당 기능 레지스터의 기본 주소이고 gpio는 핀입니다.
 */
...
현재 4 개의 인터럽트 인 에이블 레지스터에 대해서만 정의되어 있으며, 사용할 수있는 총 레귤러는 12입니다.
 */
