 ...
 * Langwell 칩은 64 핀을 가지고 있기 때문에 각 기능을 제어하기 위해 2 개의 32 비트 레지스터가 있으며 Penwell 칩은 각 블록마다 96 핀을 가지고 있으며이를 제어하기 위해 3 개의 32 비트 레지스터가 필요하므로 여기서는 구조 대신에 순서 만 정의합니다. 핀에 대해 비트 오프셋을 얻습니다 (예를 들어 GPDR 사용).
  *
  * nreg = ngpio / 32;
  * reg = 오프셋 / 32;
  * 비트 = 오프셋 % 32;
  * reg_addr = reg_base + GPDR * nreg * 4 + reg * 4;
  *
  * 그래서 reg_addr 비트는 핀 오프셋의 GPDR 기능을 제어하는 것입니다.
*/

enum GPIO_REG {
	GPLR = 0,	/* pin level read-only * 핀 레벨 읽기 전용 */
	GPDR,		/* pin direction * 핀 방향*/
	GPSR,		/* pin set * 핀 세트*/
	GPCR,		/* pin clear */
	GRER,		/* rising edge detect * 상승 에지 감지*/
	GFER,		/* falling edge detect  * 하강 에지 검출*/
	GEDR,		/* edge detect result * 가장자리 감지 결과 */
	GAFR,		/* alt function  * alt 함수*/
};
...
static void intel_mid_irq_handler(struct irq_desc *desc)
{
	...
	/* check GPIO controller to check which pin triggered the interrupt 
	   GPIO 컨트롤러를 점검하여 어떤 핀이 인터럽트를 트리거했는지 확인하십시오.	*/
			  ...
			  const struct pci_device_id *id)
{
	...
	/* release the IO mapping, since we already get the info from bar1 
	   우리가 이미 bar1로부터 정보를 얻었 기 때문에 IO 매핑을 해제하라.		*/
