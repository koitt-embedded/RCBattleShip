...
/* assumes MAX_GPIO_PER_BANK is a multiple of 2 
   MAX_GPIO_PER_BANK는 2의 배수라고 가정합니다.	*/
		...
		unsigned int enable)
{
	 ...
	 * up appropriately thanks to IRQCHIP_MASK_ON_SUSPEND flag.
	   그러나 많은 사람들이 같은 웨이크 IRQ를 사용하기 때문에 많은 사람들이 깨울 수 있습니다. 
 	   IRQCHIP_MASK_ON_SUSPEND 플래그로 인해 마스크가 적절하게 설정됩니다.
	 */
	if (enable)
...
/* Each UPG GIO block has one IRQ for all banks 
   각 UPG GIO 블록에는 모든 뱅크에 대해 하나의 IRQ가 있습니다.		*/
...
static int brcmstb_gpio_remove(struct platform_device *pdev)
{
	 ...
	 * more important to actually perform all of the steps.
	   아래에서 반환 값을 잃을 수 있지만 모든 오류를보고하므로 모든 단계를 실제로 수행하는 것이 더 중요합니다.
	 */
	list_for_each(pos, &priv->bank_list) {
...
/* Before calling, must have bank->parent_irq set and gpiochip registered 
   호출하기 전에 bank-> parent_irq가 설정되어 있고 gpiochip이 등록되어 있어야합니다.	*/
		...
		struct brcmstb_gpio_bank *bank)
{
	...
	/* Ensures that all non-wakeup IRQs are disabled at suspend
	   일시 중지시 모든 웨이크 업하지 않는 IRQ를 비활성화합니다.		 */
	...
	if (IS_ENABLED(CONFIG_PM_SLEEP) && !priv->can_wake &&
		...
		if (priv->parent_wake_irq < 0) {
			 ...
			 * (e.g., from S5 cold boot)
			   웨이크 업 인터럽트를 요청하기 전에 웨이크 업 기능을 설정하여 부트 타임
			  "웨이크 업"을 처리 할 수 있습니다 (예 : S5 콜드 부팅)
			 */
...
static int brcmstb_gpio_probe(struct platform_device *pdev)
{
	...
	of_property_for_each_u32(np, "brcm,gpio-bank-widths", prop, p,
		 ...
		 * and direction bits have 0 = output and 1 = input
		    Regs는 4 바이트 폭이고, 데이터 reg, no set / clear regs 및 방향 비트는 0 = 출력이고 1 = input
		 */
		bgc = &bank->bgc;
		...
		/* not all ngpio lines are valid, will use bank width later 
		   모든 ngpio 줄이 유효하지는 않지만 나중에 은행 가로를 사용할 것입니다	*/
		gc->ngpio = MAX_GPIO_PER_BANK;

		/*
		 * Mask all interrupts by default, since wakeup interrupts may
		 * be retained from S5 cold boot
		 * 웨이크 업 인터럽트는 기본적으로 모든 인터럽트를 마스크합니다.
		 * S5 콜드 부팅에서 유지해야합니다.
		 */
