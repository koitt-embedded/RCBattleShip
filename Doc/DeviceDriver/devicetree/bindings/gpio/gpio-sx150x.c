...
이 유틸리티 함수는 구성 비트를 찾고 설정하는 일반적인 문제를 해결합니다. 구성 비트는 인덱스가 아래로 증가하는 레지스터로 그룹화됩니다. 예를 들어 8 비트 레지스터의 경우 16 개의 gpios는 다음과 같은 순서로 그룹화 된 구성 비트를 갖습니다.
...
시작 레지스터 'N'의 주소, 구성을 변경하고자하는 gpio의 색인 및 해당 구성의 비트 단위의 너비가 주어지면이 함수를 사용하여 올바른 레지스터를 찾고 올바른 비트를 마스크 할 수 있습니다.
			...
			struct sx150x_platform_data *pdata)
{
	...
	if (chip->dev_cfg->model == SX150X_789)
		err = sx150x_i2c_write(chip멀티 비트 구성의 경우 패턴이 더 넓어집니다.->client,
