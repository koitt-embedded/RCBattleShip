...
/* List of supported Technologic Systems platforms DIO blocks 
   지원되는 Technologic Systems 플랫폼 목록 DIO 블록		*/
 ...
 * This flag ensures that the region has been requested by this driver.
  ...
  * Hex 7D는 여러 블록 (예 : DIO2 및 LCD 포트)을 제어하는 데 사용됩니다.
  * 이 플래그는이 드라이버가 영역을 요청했는지 확인합니다.
 */
 ...
 * such as available directions and connected interrupt (if any).
  ...
  * 이 구조는 DIO 라인의 기능을 설명하는 데 사용됩니다.
  * 가능한 방향 및 연결된 인터럽트 (있는 경우).
 */
struct ts5500_dio {
	const u8 value_addr;
	const u8 value_mask;
...
입력 / 출력 DIO 라인은 4 개의 그룹으로 프로그래밍됩니다. 이들 값은 값 포트에서 4 개의 연속 비트를 통해 사용할 수 있지만,이 4 개 라인의 방향은 제어 포트에서 단 1 비트로 구동됩니다.
 */
...
static int ts5500_gpio_to_irq(struct gpio_chip *chip, unsigned offset)
{
	...
	/* As this pin is input-only, we may strap it to another in/out pin
	   이 핀은 입력 전용이므로, 다른 입출력 핀에 결선 할 수 있습니다 		*/
	if (priv->strap)
		return priv->hwirq;
