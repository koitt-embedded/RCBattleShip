...
struct mpc8xxx_gpio_chip {
	 ...
	 * open drain mode safely
	   오픈 드레인 모드의 출력 핀을 안전하게 지우거나 설정할 수있는 섀도우 데이터 레지스터
	 */
	u32 data;
...
해결 방법 GPIO 1 정오표 MPC8572 / MPC8536. 출력으로 정의 된 GPIO의 상태는 GPDAT 레지스터를 읽음으로써 결정될 수 없으므로 대신에 섀도우 데이터 레지스터를 사용합니다. 입력 핀의 상태는 GPDAT 레지스터를 읽음으로써 결정됩니다.
 */
...
static struct irq_chip mpc8xxx_irq_chip = {
	...
	/* this might get overwritten in mpc8xxx_probe()
	   이것은 mpc8xxx_probe ()에서 덮어 쓸 수 있습니다.	 */
...
static int mpc8xxx_probe(struct platform_device *pdev)
{
	 ...
	 * on the current machine, so overwriting global data is fine.
	   현재 컴퓨터에서 사용할 수있는 유형은 gpio 컨트롤러 한 종류 뿐이므로 전역 데이터 덮어 쓰기가 문제가되지 않습니다.
	 */
