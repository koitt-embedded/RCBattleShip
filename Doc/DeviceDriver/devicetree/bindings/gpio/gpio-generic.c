 ...
 *             .. the registers. ,............```.`.`..`.`.~~~.`.`.`~
 *               `.......````.```
드라이버가 지원하는 가장 간단한 형태의 GPIO 컨트롤러는 GPIO 상태를 읽고 쓸 수있는 단 하나의 "데이터"레지스터 일뿐입니다. FPGA에서 그러한 GPIO 컨트롤러를 구현하는 것은 쉽지 않습니다. 드라이버는 빅 엔디안 표기법을 지원합니다. 조금 더 정교한 컨트롤러는 데이터 레지스터에 영향을주는 세트 / 클리어 비트 레지스터 접미사 쌍과 함께 장치를 등록하며 출력 핀도 지원됩니다. 예상되는 것은 적어도 일부는 이 경우는 Verilog의 자체 롤 ASIC / FPGA 로직과 함께 사용되거나 VHDL 8, 16, 32 및 64 비트 레지스터가 지원되며 GPIO 수는 레지스터의 너비에 따라 결정됩니다.
 */
 ...
 *	the GPIO is an input.
  * 장치를 만들고 자원을 할당하십시오. GPIO 설정에는 세 가지 지원되는 구성이 있습니다.
  *
  * - 단일 입출력 레지스터 자원 ( "dat").
  * - set / clear 쌍 ( "set"및 "clr").
  * - 단일 출력 레지스터 자원 및 단일 입력 자원 ( "set"및 dat ").
  *
  * 단일 출력 레지스터의 경우 비트를 지워서 1을 출력하고 비트를 지워서 0을 출력합니다.
    set clr pair의 경우 set 레지스터에서 비트를 설정하여 1을 구동하고 clear 레지스터의 비트를 설정하여 비트를 지 웁니다.
  * 구성은 어떤 자원이 있는지에 의해 감지됩니다.
  *
  * GPIO 방향 설정에는 세 가지 지원되는 구성이 있습니다.
  *
  * - 구성이 필요없는 간단한 양방향 GPIO.
  * - 출력 방향 레지스터 ( "dirout"). 1 비트는 GPIO가 출력임을 나타냅니다.
  * - 입력 방향 레지스터 ( "dirin"이라고 함). 여기서 1 비트는 GPIO가 입력임을 나타냅니다.
 */
