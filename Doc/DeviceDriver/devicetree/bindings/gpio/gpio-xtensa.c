 ...
 * Xtensa LX4 GPIO32 옵션 용 드라이버
 *
 * 설명서 : Xtensa LX4 마이크로 프로세서 데이터 북, 섹션 2.22
 *
 * GPIO32는 Xtensa 아키텍처 코어의 표준 옵션 확장 기능으로, 인트라 SoC 시그널링을 위해 사전 구성된 출력 및 입력 포트를 제공합니다. GPIO32 옵션은 EXPSTATE라고하는 32 비트 텐 실리카 명령 확장 (TIE) 출력 상태와 IMPWIRE라는 32 비트 입력 와이어로 구현됩니다. 이 드라이버는 입력 및 출력 상태를 두 개의 다른 장치로 처리합니다.
 *
 * GPIO32 특정 명령에 대한 액세스는 CPENABLE (Coprocessor Enable Bits) 레지스터에 의해 제어됩니다. 기본적으로 Xtensa Linux 시작 코드는 모든 보조 프로세서에 대한 액세스를 비활성화합니다. 이 드라이버는 GPIO32 특정 명령보다 먼저 GPIO32에 해당하는 CPENABLE 비트를 설정하고 그 후에 CPENABLE 상태를 복원합니다.
 *
 *이 드라이버는 현재 SMP와 호환되지 않습니다. GPIO32 확장이 모든 코어에서 사용 가능하다는 보장은 없습니다. 또한 각 코어는 다른 IO 와이어 세트를 제어합니다. 이 드라이버의 이론적 인 SMP 인식 버전은 실제 GPIO 조작을 수행하기 위해 코어 당 워크 큐가 있어야합니다.
 */
