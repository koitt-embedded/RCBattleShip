...
/ **
  * it87_gpio 구조체 - it87 관련 GPIO 칩
  * @chip 밑에있는 gpio_chip 구조체를칩니다.
    작업 사이의 경쟁을 피하기 위해 잠금을 잠급니다.
  * gio 포트의 @io_base 기본 주소
  * @io_size io_base에서 시작하는 포트 분노의 크기입니다.
  * @output_base 출력 인 에이블 레지스터의 수퍼 I / O 레지스터 주소
  * @simple_base 수퍼 I / O '단순 I / O'가능 레지스터
  * @simple_size 수퍼 IO '단순 I / O'레지스터 크기 사용; IT87xx 칩은 회선의 서브 세트에 단순 I / O 스위치를 제공 할 수 있기 때문에 필수적입니다. 반면 다른 제품은 동일한 상태를 항상 유지합니다.
 */
struct it87_gpio {
...
/* Superio chip access functions; copied from wdt_it87 
    Superio 칩 접근 기능; wdt_it87에서 복사		*/

static inline int superio_enter(void)
{
	/*
	 * Try to reserve REG and REG + 1 for exclusive access.
	   독점적 인 액세스를 위해 REG 및 REG + 1을 예약하십시오.
	 */
...
static int it87_gpio_request(struct gpio_chip *chip, unsigned gpio_num)
{
	 ...
	 * them allow all the lines to be set/unset to Simple I/O.
	    모든 IT87xx 칩이 Simple I / O를 지원하는 것은 아니며 모든 라인이 Simple I / O를 설정 / 해제 할 수있는 것은 아닙니다.
	 */
	 ...
	 * newly-exported GPIO interfaces are set to input.
	    모든 출력 된 GPIO 인터페이스가 입력으로 설정되어 있기 때문에 핀을 입력으로 설정하여 출력을 활성화하십시오.
	 */
...
static int __init it87_gpio_init(void)
{
	 ...
	 * to which GPIO pin we're referring to.
 	  * GPIO 연결에 대한 별칭을 설정하십시오.

	  * IT8728F와 같은 최신 칩에 대한 ITE 문서는 GPIO 라인을 GPxy로, 좌표계가 x가 GPIO 그룹 (1부터 시작)이고 y가 그룹 내의 비트임을 나타냅니다.
	
	  * 이러한 별칭을 만들어서 우리가 참조하고있는 GPIO 핀을 더 쉽게 이해할 수 있습니다.
	 */
