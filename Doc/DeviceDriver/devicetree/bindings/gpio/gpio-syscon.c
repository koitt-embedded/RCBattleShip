...
/* SYSCON driver is designed to use 32-bit wide registers 
   SYSCON 드라이버는 32 비트 폭 레지스터를 사용하도록 설계되었습니다.	*/
 ...
 *			for signal "offset"
    ...
    struct syscon_gpio_data - 장치의 설정.
  * compatible : SYSCON 드라이버 호환 문자열.
  * flags : GPIO_SYSCON_FEAT_ 플래그 세트 :
  * GPIO_SYSCON_FEAT_IN : GPIO는 입력,
  * GPIO_SYSCON_FEAT_OUT : GPIO는 출력,
  * GPIO_SYSCON_FEAT_DIR : GPIO는 스위치 방향을 지원합니다.
  * bit_count : GPIO로 사용되는 비트 수.
  * dat_bit_offset : 첫 번째 GPIO 비트에 오프셋 (비트).
  * dir_bit_offset : GPIO 방향을 전환하기위한 첫 번째 비트에 대한 선택적 오프셋 (비트 단위) (GPIO_SYSCON_FEAT_DIR 플래그와 함께 사용).
  * set : 신호 "offset"에 대한 출력 값을 지정하는 HW 특정 콜백
 */
