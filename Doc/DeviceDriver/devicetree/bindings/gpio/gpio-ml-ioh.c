 ...
 * @use_sel_reg: To store contents of GPIO_USE_SEL0~3
  ...
  struct ioh_gpio_reg_data - 레지스터 저장 데이터.
  * @ien_reg 인터럽트 허가 레지스터의 내용을 저장한다.
  * @imask_reg : 인터럽트 마스크 등록 내용 저장
  * @po_reg : PO 레지스터의 내용을 저장합니다.
  * @pm_reg : PM 레지스터의 내용을 저장합니다.
  * @ im0_reg : 인터럽트 모드의 내용을 저장하는 레지스터입니다.
  * @ im1_reg : 인터럽트 모드의 내용을 저장합니다. regist1
  * @use_sel_reg : GPIO_USE_SEL0 ~ 3의 내용을 저장합니다.
 */
 ...
 * @spinlock:		Used for register access protection
  * struct ioh_gpio - GPIO 개인 데이터 구조.
  * @base : 메모리 매핑 I / O 레지스터의 PCI 기본 주소.
  * @reg : 메모리 매핑 된 IOH GPIO 레지스터 목록.
  * @dev : 장치 구조의 포인터.
  * @gpio : GPIO 인프라 스트럭처 용 데이터.
  * @ioh_gpio_reg : 메모리 매핑 된 레지스터 데이터가 여기에 저장됩니다.
  * 일시 중지.
  * @gpio_use_sel : PM에 GPIO_USE_SEL1 ~ 4 레지스터 저장
  * @ch : GPIO 채널 표시
  * @irq_base : 인터럽트를 위해 IRQ 번호의 기본 값을 저장합니다.
  * @spinlock : 레지스터 액세스 보호에 사용됩니다.
 */
struct ioh_gpio {
	void __iomem *base;
