...
/**
 * MCP types supported by driver 드라이버가 지원하는 MCP 유형
 */
 ...
 * with either 16 bit registers or with two adjacent 8 bit banks.
  레지스터는 모두 8 비트 폭입니다.
  * mcp23s17은 두 배의 비트를 가지며 16 비트 레지스터 또는 인접한 두 개의 8 비트 뱅크로 작동하도록 구성 할 수 있습니다.
 */
 ...
 * Driver data holds all the per-chip data.
  주어진 spi_device는 동일한 칩 선택을 공유하지만 다른 주소를 사용하는 최대 8 개의 mcp23sxx 칩을 나타낼 수 있습니다 (예 : 칩 # 0 및 # 3이 채워질 수 있지만 # 1 또는 $ 2는 채워지지 않을 수 있음). 드라이버 데이터는 모든 칩당 데이터를 보유합니다.
 */
 ...
 * pullups, deglitching, open drain drive.
  이것은 일반 gpio 덤프 코드보다 많은 정보를 보여줍니다 : pullups, deglitching, open drain drive.
 */
			      ...
			      struct mcp23s08_platform_data *pdata, int cs)
{
	 ...
	 * and MCP_IOCON.HAEN = 1, so we work with all chips.
	   MCP_IOCON.SEQOP = 0을 확인하면 순차 읽기가 가능하고 MCP_IOCON.HAEN = 1이므로 모든 칩에 대해 작업 할 수 있습니다.
	 */
	...
	if ((status & IOCON_SEQOP) || !(status & IOCON_HAEN) || mirror ||
		...
		/* mcp23s17 has IOCON twice, make sure they are in sync
	  	   mcp23s17은 IOCON을 두 번 사용합니다. 동기화되어 있는지 확인하십시오. */
...
/* NOTE: The use of the mcp prefix is deprecated and will be removed. 
   참고 : mcp 접두어는 더 이상 사용되지 않으며 제거됩니다.			*/
	{
...
static int mcp23s08_probe(struct spi_device *spi)
{
	 ...
	 * handled here...
	   참고 :이 칩은 신호 당 마스킹 및 레벨 / 에지 트리거링과 함께 상대적으로 정상적인 IRQ 프레임 워크를 사용합니다. 여기 아직 처리되지 않았습니다 ...
	 */
 ...
 * subsys initcalls that may rely on these GPIOs
   spi / i2c postcore initcall 이후와이 GPIO에 의존 할 수있는 서브 시스템 시작 전에 등록하십시오.
 */
