 ...
 * asserted.
 인터럽트가 발생한 후 이벤트 카운터 레지스터가 25ms 후에 업데이트되었으므로 판독을 적어도 25ms 지연시키는 데 필요한 초기 사전 4.0 실리콘.
 */
...
static void adp5588_irq_bus_lock(struct irq_data *d)
{
  ...
  * and unlocks the bus.
   * genirq 핵심 코드는 원자 문맥에서 칩 - 마스크 / 언 마스를 실행할 수 있습니다.
   * 액세스가 잠길 필요가있는 느린 버스에서는 작동하지 않습니다.
   * bus_sync_unlock ()은 atomic 컨텍스트 외부에서 호출되어 현재 irq 마스크 상태를 저속 외부 컨트롤러와 동기화하고 버스를 잠금 해제합니다.
  */
