 ...
 *  published by the Free Software Foundation.
  * 74Hx164 - 일반 직렬 입력 / 병렬 출력 8 비트 시프트 레지스터 GPIO 드라이버
  *
  * Copyright (C) 2010 Gabor Juhos <juhosg@openwrt.org>
  * Copyright (C) 2010 Miguel Gaio <miguel.gaio@efixo.com>
  *
  * 이 프로그램은 무료 소프트웨어입니다. 
    자유 소프트웨어 재단이 발표 한 GNU General Public License 버전 2의 조건에 따라 재배포 및 / 또는 수정할 수 있습니다.
 */
...
static int __gen_74x164_write_config(struct gen_74x164_chip *chip)
{
	 ...
	 * byte of the buffer will end up in the last register.
 	   레지스터가 체인으로 연결되어 있기 때문에 전송 된 모든 바이트는 이전 바이트를 체인의 다음 레지스터로 이동합니다. 
	   따라서 첫 번째 바이트 전송은 전송이 끝날 때 마지막 레지스터에서 끝납니다.
 	   따라서 논리적 번호 매김을하려면 버퍼의 마지막 바이트가 마지막 레지스터에서 끝나도록 바이트를 역순으로 보내야합니다.
	 */
