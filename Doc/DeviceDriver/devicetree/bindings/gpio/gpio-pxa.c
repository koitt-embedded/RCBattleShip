...
우리는 bank에서 GPIO를 처리하며 각 bank은 하나의 레지스터 세트로 최대 32 개의 GPIO를 처리합니다. 레지스터 오프셋은 다음과 같이 구성됩니다.
 *
...
PXA26x의 GPIO86 / 87 / 88 / 89는 PXA_GPDR에서 방향 비트를 가지고 있습니다 (2는 거꾸로되어 있으며 GAFRx의 GPIO는 대체 기능 값이 '1'임).
 */
...
PXA25x 및 PXA27x에서 GAFRx와 GPDRx는 함께 GPIO의 대체 기능을 결정하며 GPDRx는 일단 구성되면 변경할 수 없습니다. 여기에 "점령 된"것으로 간주됩니다 (이 용어가 정확하지 않음을 알고 있습니다. 더 나은 것을 제안 할 수 있습니다 :-)
 */
					...
					int (*set_wake)(unsigned int, unsigned int))
{
	...
	for (i = 0, gpio = 0; i < nbanks; i++, gpio += 32) {
		...
		/* number of GPIOs on last bank may be less than 32
		   마지막 뱅크의 GPIO 수는 32보다 작을 수 있습니다.	 */
 ...
 * bits are set in c->irq_mask
   해당 비트가 c-> irq_mask에 설정되어 있으면 해당 GRERx 및 GFERx 에지 감지 레지스터 비트 만 업데이트하십시오.
 */
...
static int pxa_gpio_irq_type(struct irq_data *d, unsigned int type)
{
	...
	if (type == IRQ_TYPE_PROBE) {
		 ...
		 * GPIOs set to alternate function or to output during probe
	     	   사전 구성된 에지 또는 GPIO가 대체 기능으로 설정되거나 프로브 도중 출력되도록 설정된 GPIO를 사용하지 마십시오.
		 */
...
static int pxa_gpio_probe(struct platform_device *pdev)
{
	...
	/* Initialize GPIO chips  GPIO 칩 초기화	*/
	pxa_init_gpio_chip(pxa_last_gpio, info ? info->gpio_set_wake : NULL);

	/* clear all GPIO edge detects  모든 GPIO 에지 감지를 지 웁니다	*/
	for_each_gpio_chip(gpio, c) {
		writel_relaxed(0, c->regbase + GFER_OFFSET);
		writel_relaxed(0, c->regbase + GRER_OFFSET);
		writel_relaxed(~0, c->regbase + GEDR_OFFSET);
		/* unmask GPIO edge detect for AP side	 AP 측의 GPIO 에지 검출을 해제합니다.	 */
