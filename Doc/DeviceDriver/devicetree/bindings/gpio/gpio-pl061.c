		...
		int value)
{
	 ...
	 * a gpio pin before configuring it in OUT mode.
	  gpio 값은 다시 설정됩니다. 왜냐하면 pl061은 OUT 모드에서 구성하기 전에 gpio 핀의 값을 설정할 수 없기 때문입니다.
	 */
	writeb(!!value << offset, chip->base + (BIT(offset + 2)));
	spin_unlock_irqrestore(&chip->lock, flags);
...
static int pl061_irq_type(struct irq_data *d, unsigned trigger)
{
	...
	if (trigger & (IRQ_TYPE_LEVEL_HIGH | IRQ_TYPE_LEVEL_LOW)) {
		...
		/* Disable edge detection 가장자리 감지 사용 안 함*/
		gpioibe &= ~bit;
		/* Enable level detection  레벨 감지 사용*/
		gpiois |= bit;
		/* Select polarity  극성 선택*/
		if (polarity)
			gpioiev |= bit;
		else
			gpioiev &= ~bit;
		...
		/* Disable level detection  레벨 감지 비활성화*/
		gpiois &= ~bit;
		/* Select both edges, setting this makes GPIOEV be ignored 
		   양쪽 가장자리를 선택하고, GPIOEV를 무시하도록 설정합니다.	*/
		gpioibe |= bit;
		...
		/* Disable level detection 레벨 감지 비활성화 */
		gpiois &= ~bit;
		/* Clear detection on both edges 양쪽 가장자리의 지우기 감지*/
		gpioibe &= ~bit;
		/* Select edge 가장자리 선택*/
		if (rising)
			gpioiev |= bit;
 ...
 */
/**
  * pl061_irq_ack () - 가장자리 IRQ ACK
  * @d :이 IRQ의 IRQ 데이터
  *
  * 이것은 가장자리 IRQ 처리기에서 호출되어 가장자리 IRQ를 ACK합니다.
  * GPIOIC (인터럽트 - 클리어) 레지스터. 레벨 IRQ의 경우 이것은
  * 필요 없음 : 레벨 신호가 사라지면 사라집니다.
  */
