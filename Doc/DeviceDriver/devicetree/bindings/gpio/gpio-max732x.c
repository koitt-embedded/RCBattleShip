 ...
 * MAX732x의 각 포트 (MAX7319 포함)는 다음 세 가지 유형 중 하나에 속한다.
 *
 * - 푸시 풀 출력
 * - 입력
 * - 오픈 드레인 I / O
 *
 * MAXIM의 데이터 시트에 따라 'O', 'I'및 'P'로 개별적으로 지정됩니다. 'I'및 'P'포트는 인터럽트 가능 케이블이며 일부는 전용 인터럽트 마스크가 있습니다.
 *
 * 두 그룹의 I / O 포트가 있으며 각 그룹은 일반적으로 최대 8 개의 I / O 포트를 포함하며 특정 I2C 주소로 액세스됩니다.
 *
 * - 그룹 A : I2C 주소 0b'110xxxx에 의한
 * - 그룹 B : I2C 주소 별 0b'101xxxx
 *
 * 여기서 'xxxx'는 핀 AD2 / AD0의 연결에 의해 결정됩니다. 사용 된 주소는 출력 신호의 초기 상태에도 영향을 미칩니다.
 *
 * 각 포트 그룹에는 알려진 I / O 포트 조합이 5 개 있습니다 (4I4O, 4P4O, 8I, 8P, 8O). 자세한 포트 구성은 아래 정의를 참조하십시오. Goup A 만 인터럽트 가능합니다.
 *
 * GPIO 번호는 'gpio_base + 0'에서 'gpio_base + 8/16'까지이며 GROUP_A의 GPIO는 GROUP_B의 GPIO보다 앞에 번호가 매겨집니다 (두 개의 그룹이있는 경우).
 *
 * 참고 : MAX7328 / MAX7329는 PCF8574 / a의 드롭 인 대체품이므로이 드라이버에서 지원되지 않습니다.
 */
...
static int max732x_gpio_direction_input(struct gpio_chip *gc, unsigned off)
{
	 ...
	 * equivalent to output-high) to be turned into an input.
	   오픈 드레인 핀은 하이 임피던스 (출력 하이에 해당)로 설정해야 입력으로 바뀔 수 있습니다.
	 */
