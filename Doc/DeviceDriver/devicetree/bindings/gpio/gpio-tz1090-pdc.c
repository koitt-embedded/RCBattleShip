...
이것은 PDC GPIO 드라이버의 주요 개인 데이터입니다. gpio_chip을 캡슐화하고 gpio_chip의 콜백은 아래의 to_pdc () 매크로를 사용하여 개인 데이터에 액세스 할 수 있습니다.
