...
/*
  * 메모리 레이아웃 :
  *이 칩은 4 개의 GPIO 섹션을 가지고 있으며 각 GPIO는 8 개의 GPIO를 제어합니다.
  * 섹션 0의 비트 0은 GPIO 0이며, 섹션 1의 비트 2는 GPIO 10입니다.
  * 면책 조항 : 리버스 엔지니어링!
  * 자세한 내용은 다음을 참조하십시오 :
  * http://hackspire.unsads.com/wiki/index.php/Memory-mapped_I/O_ports#90000000_-_General_Purpose_I.2FO_.28GPIO.29
  *
  * 0x00-0x3F : 섹션 0
  * + 0x00 : 마스크 된 인터럽트 상태 (읽기 전용)
  * + 0x04 : R : 인터럽트 상태 W : 인터럽트 상태를 리셋
  * + 0x08 : R : 인터럽트 마스크 W : 마스크 인터럽트
  * + 0x0C : W : 마스크 해제 인터럽트 (쓰기 전용)
  * + 0x10 : 방향 : I / O = 1 / 0
  * + 0x14 : 출력
  * + 0x18 : 입력 (읽기 전용)
  * + 0x20 : R : 레벨 인터럽트 W : 레벨 인터럽트로 설정
  * 0x40-0x7F : 섹션 1
  * 0x80-0xBF : 섹션 2
  * 0xC0-0xFF : 섹션 3
*/
...
/* Bit number of GPIO in its section	 해당 섹션의 GPIO 비트 수	 */
...
static int zevio_gpio_probe(struct platform_device *pdev)
{
	...
	/* Disable interrupts, they only cause errors
	   인터럽트를 사용 안 함으로 설정하면 오류 만 발생합니다.	 */
