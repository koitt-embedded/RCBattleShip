...
/* Mapping betwee numeric GPIO ID and the actual GPIO hardware numbering:
   숫자 GPIO ID와 실제 GPIO 하드웨어 번호 매기기 간의 매핑 :
...
static int vx855gpio_get(struct gpio_chip *gpio, unsigned int nr)
{
	...
	if (nr < NR_VX855_GPI) {
		 ...
		 * back from the output register
	   	 GPO에 입력 비트가 없으므로 출력 레지스터에서 다시 읽어야합니다. */
				      ...
				      unsigned int nr, int val)
{
	 ...
	 * so all we do is set the level 
	   ...
	   True GPO는 출력 모드로 전환 할 필요가 없으며 GPIO는 오픈 드레인이므로 스위칭이 필요하지 않으므로 레벨을 vx855gpio_set (gpio, nr, val)로 설정하면됩니다.	*/
...
/* This platform device is ordinarily registered by the vx855 mfd driver 
   이 플랫폼 장치는 일반적으로 vx855 mfd 드라이버에 의해 등록됩니다.			*/
...
단일 바이트는 VX855의 다양한 GPIO 포트를 제어하는 데 사용되고 OLPC XO-1.5의 경우 일부 포트는 ACPI를 통해 해석되고 노출되는 스위치에 사용됩니다. ACPI는이 지역을 예약 할 것이므로 우리 자신의 예약은 성공하지 못할 것입니다. 무시하고 계속하십시오.
	 */
