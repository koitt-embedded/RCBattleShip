...
static int adnp_irq_setup(struct adnp *adnp)
{
	 ...
	 * each segment contains the corresponding bits for all interrupts.
	   인터럽트의 현재 레벨과 트리거 모드를 추적하기 위해 메모리를 할당하십시오.
           다중 할당을 피하기 위해 하나의 큰 버퍼가 할당되고 포인터가 해당 오프셋을 가리 키도록 설정됩니다.
	   일관성을 위해, 버퍼의 레이아웃은 각 세그먼트가 모든 인터럽트에 대응하는 비트를 포함한다는 점에서 하드웨어의 레지스터 레이아웃과 일치하도록 선택됩니다.	
	 */
	...
	for (i = 0; i < num_regs; i++) {
		 ...
		 * of edge triggered interrupts.
		   에지 트리거 인터럽트의 에뮬레이션을 허용하기 위해 모든 핀의 초기 레벨을 읽습니다
		 */
