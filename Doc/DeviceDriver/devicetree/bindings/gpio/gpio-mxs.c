...
/* Note: This driver assumes 32 GPIOs are handled in one register
   참고 :이 드라이버는 하나의 레지스터에서 32 개의 GPIO가 처리된다고 가정합니다. */
...
/* MXS has one interrupt *per* gpio port
   MXS에는 * gpio 포트 당 하나의 인터럽트가 있습니다.	 */
...
static int mxs_gpio_probe(struct platform_device *pdev)
{
	 ...
	 * share the same one
	   모든 gpio 포트가 동일한 메모리 영역을 공유하기 때문에 메모리 영역을 한 번만 매핑하십시오
	 */
	if (!base) {
	 ...
	 * disable the interrupts
	   핀 인터럽트 기능을 선택하지만 초기에는 인터럽트를 비활성화한다.
	 */
	writel(~0U, port->base + PINCTRL_PIN2IRQ(port));
	writel(0, port->base + PINCTRL_IRQEN(port));

	/* clear address has to be used to clear IRQSTAT bits 
	   IRQSTAT 비트를 지우려면 클리어 주소를 사용해야한다.	*/
	...
	/* gpio-mxs can be a generic irq chip 
	   gpio-mxs는 일반적인 irq 칩이 될 수 있습니다.	*/
