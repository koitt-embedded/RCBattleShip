...
* Lynxpoint gpios는 비트 맵 레지스터와 gpio 특정 레지스터를 통해 제어됩니다. 비트 맵 레지스터는 3 x 32 비트 덩어리로 구성되어 있으며 gpio 당 모든 94 gpios는 gpio 당 2 개의 32 비트 레지스터 (LP_CONFIG1 및 LP_CONFIG2)로 구성되며, 94 gpios에는 총 188 개의 구성 레지스터가 있습니다.
  *
  * 레지스터 레이아웃의 간략한보기는 다음과 같습니다.
...
static void lp_gpio_irq_handler(struct irq_desc *desc)
{
	...
	/* check from GPIO controller which pin triggered the interrupt
	    어떤 핀이 인터럽트를 트리거했는지 GPIO 컨트롤러에서 확인		 */
...
static int lp_gpio_resume(struct device *dev)
{
	...
	/* on some hardware suspend clears input sensing, re-enable it here
	   일부 하드웨어 일시 중단 입력 감지를 지우고, 여기 다시 활성화하십시오	 */
